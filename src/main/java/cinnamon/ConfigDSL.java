...
public final class ConfigDSL {
    ...
    public static class Config {
        ...
        public double get(String name, int lvl) {
            return get(name, lvl, 1D);
        }

        public double get(String name, int lvl, double effective) {
            double result = 0D;
            try {
                ...
                result *= effective;
                result = clamp(result, clampLeft, clampRight);
            } catch (Exception e) {
                e.printStackTrace();
                result = 0D;
            }
    ...
    public static final BiFunction<Property, String[], Curve> CURVE_RAMP = (property, strings) -> {
        ...
        switch (strings[0]) {
            ...
            case RAMP_NEGATIVE:
                ...
                break;
            default:
                strength = parseDoubleFromString(property, strings[0]) / 100D;
                position = parseDoubleFromString(property, strings[1]) / 100D;
        }
    ...
    public static final Function<String[], Curve> CURVE_FUNCTION = strings -> {
        ...
        return (min, max, start, end, n) -> {
            String pureFunction = function
                    ...
                    .replace("{max}", "y")
                    .replace("{level}", "l");
            return ExpressionHelper.getExpression(pureFunction, min, max, n);
        };
    };
    ...
    public static double parseDoubleFromString(Property property, String string) {
        ...
        if (string.endsWith(PERCENTAGE)) {
            return Double.parseDouble(string.substring(0, string.indexOf(PERCENTAGE)));
        }
        if (string.endsWith(SECONDS)) {
            return Double.parseDouble(string.substring(0, string.indexOf(SECONDS)));
        }
        if (string.endsWith(BLOCKS)) {
            return Double.parseDouble(string.substring(0, string.indexOf(BLOCKS)));
        }
        if (string.endsWith(HEART)) {
            return Double.parseDouble(string.substring(0, string.indexOf(HEART)));
        }
